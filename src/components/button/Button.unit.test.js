...
test('should render title', async () => {
  await render(<Button title="My Button" />);
...
test('should fire onClick event handler', async () => {
  ...
  const { user } = await render(<Button title="My Button" onClick={onClick} />);
...
test('button should get a class', async () => {
  await render(<Button title="My Button" className="my-class" />);

  // The button has 'my-class' class
  expect(screen.getByRole('button')).toHaveClass('my-class');
});

test('default "size" prop should be "big"', async () => {
  await render(<Button title="My Button" />);
...
test('should be effected by "size" prop', async () => {
  await render(
    <>
...
test('default "theme" prop should be "green"', async () => {
  await render(<Button title="My Button" />);
...
test('should be effected by "theme" prop', async () => {
  await render(
    <>
...
test('should get style by props', async () => {
  const style = { borderColor: 'red', height: '10px', width: '10px' };
  await render(<Button title="My Button" style={style} />);
